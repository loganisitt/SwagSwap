...
public class MessageAdapter extends ParseQueryAdapter<ParseObject> {
    ...
    public View getItemView(ParseObject object, View v, ViewGroup parent) {
        if (v == null) {
            v = View.inflate(getContext(), R.layout.message_list, null);
        }
        ...
        // Add the title view
       final  TextView titleTextView = (TextView) v.findViewById(R.id.sender_name);
        final ParseImageView pro = (ParseImageView) v.findViewById(R.id.pro_pic);
       // titleTextView.setText(object.getString("sender"));
        final ParseUser sender = object.getParseUser("sender");
        sender.fetchInBackground(new GetCallback<ParseObject>() {
            @Override
            public void done(ParseObject parseObject, ParseException e) {
               // titleTextView.setTextColor();
                titleTextView.setText(sender.getString("name"));
                ParseFile IMG = sender.getParseFile("picture");

                pro.setParseFile(IMG);
                pro.loadInBackground();
